/*
 * Fixed test program for pigpio GPIO backend
...
// Fixed pigpio GPIO functions
static bool pigpio_initialized = false;

int gpio_export(int n) {
    printf("[PIGPIO] gpio_export(%d)\n", n);
    if (!pigpio_initialized) {
        // Try to initialize pigpio with different methods
        int ret = gpioInitialise();
        if (ret < 0) {
            printf("❌ Failed to initialize pigpio (error: %d)\n", ret);
            printf("💡 Trying alternative initialization...\n");
            
            // Try with sudo privileges or different method
            ret = gpioInitialise();
            if (ret < 0) {
                printf("❌ Alternative initialization also failed (error: %d)\n", ret);
                printf("💡 Please check if pigpiod daemon is running:\n");
                printf("   sudo systemctl status pigpiod\n");
                printf("   sudo systemctl start pigpiod\n");
                return -1;
            }
        }
        pigpio_initialized = true;
        printf("✅ pigpio initialized successfully (version: %d)\n", gpioVersion());
    }
    return 0;
}
...
{
    printf("\n=== Enhanced GPIO Export Tests (pigpio fixed) ===\n");
...
{
    printf("\n=== Enhanced GPIO Direction Tests (pigpio fixed) ===\n");
    double start_time = get_time_ms();
...
{
    printf("\n=== Enhanced GPIO Value Tests (pigpio fixed) ===\n");
    double start_time = get_time_ms();
...
{
    printf("\n=== GPIO Performance Tests (pigpio fixed) ===\n");
    double start_time = get_time_ms();
...
{
    printf("\n=== GPIO Stress Tests (pigpio fixed) ===\n");
    double start_time = get_time_ms();
...
{
    printf("\n=== GPIO Cleanup Tests (pigpio fixed) ===\n");
    double start_time = get_time_ms();
...
int main(void)
{
    printf("=== pigpio GPIO Test Program (Fixed) ===\n");
    printf("Using system pigpio library (version: %d)\n", gpioVersion());
